...
> ![http://wiki.labstreaminglayer.googlecode.com/hg/media/screenshots/curiarecorder-study.png](http://wiki.labstreaminglayer.googlecode.com/hg/media/screenshots/curiarecorder-study.png)-->

# Misc Features

The CuriaRecorder has some useful features that can add robustness if things go wrong during the experiment:
...
If a device is displayed in red when you start recording (and it is checked), it will be added to the ongoing recording by the time when it comes online. This can be useful when a device can only be turned on while the recording is already in progress. Again, it is advisable to check that the device is in fact discoverable and added. The CuriaRecorder brings up a console window in the background which shows a list of all streams that are added to the recording -- this is a good place to check whether a late stream did get picked up successfully during a live recording.
